...
function LiquidGlassButton({
  ...
  const cls = cn("relative", liquidbuttonVariants({ variant, size, className }))

  const inner = (
    <>
      {/* Glass shell — inset shadow stack mimics refraction + depth */}
      ...
      <div
        ...
        style={{ backdropFilter: 'url("#lgb-glass-filter")' }}
      />
      <div className="pointer-events-none relative z-10 inline-flex items-center gap-2">{children}</div>
      <GlassFilter />
    </>
  )
  ...
  return (
    <button data-slot="button" className={cls} {...props}>
      {inner}
    </button>
  )
}
...
export { Button, buttonVariants, liquidbuttonVariants, LiquidGlassButton as LiquidButton }
